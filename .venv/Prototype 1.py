...
def place_next_item(grid,items,current,placed_items):
    row = 0
    col = 0
    found = False
    end = False
    while not found and not end:
        if grid[row][col] != 'X':
            ...
            if result:
                found = True
                grid = place_item(items[current], temp_pos, grid)
    ...
    if current < len(items):
        place_next_item(grid, items, current, placed_items)
    return placed_items
...
grid = [['O']*dimensions[1] for i in range(dimensions[0])]
placed_items = place_next_item(grid, items, 0, placed_items)
